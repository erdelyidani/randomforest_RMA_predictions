...
#Small version:
ori <- SpatialPoints(cbind(-835000, 3500000), proj4string =  CRS("+init=epsg:3857"))
...
#get elevation data to grid cells:
GRID_coordinates = data.frame(coordinates(GRID))


GRID_coordinates$x <- GRID_coordinates$x - 5000
...
#remove 5km from all the coordinates, because the rounding process messed it.
...
library(rgdal)
setwd("C:/Users/Daniel/Google Drive/Erdélyi Dániel/Doktori/Alaa/Map_KG-Global")
...
full_Data  <-readxl::read_excel("Interpre_RMA+QC - final.xlsx",col_names=TRUE, na="NA", sheet="final")
colnames(full_Data) <- c(  "Valid. Categ."   ,         "Country"     ,             "WMO Code"   ,              "Site"     ,                "Latitude"   ,             
                           "Longitude",                "X",                        "Y",                        "Altitude",                 "All time O",              
                           "All time H",               "Focus period O",           "Focus period H",           "RMA_slope",                "RMA_intercept",           
                           "r2",                       "p",                        "Nr of pairs (O,H)",        "Nr of Months",             "Data from how many years",
                           "QC"  )
...
#remove Acoruna, Porto, Penhas from full data ( they are outside of the new GRID boundary and the Hengl method cannot run down)

Train_Data = full_Data %>% dplyr::select(RMA_slope,X,Y, KG) # response variables (RMA slope / RMA intercept) and other predictors.
...
#set 1:
#Train_Data <- Train_Data[-c(18,25), ]


#Set 2:
#Train_Data <- Train_Data[-c(2, 19, 26), ]


#final:
Train_Data <- Train_Data[-c(18, 25, 49), ]
##############################################################################################
...
RF2 = rfsrc( RMA_slope ~ ., data = as.data.frame(Train_Data),
             ...
             mtry = TuneRF2$optimal['mtry'], importance = TRUE)
plot(RF2)
#plot(get.tree(RF2, 3))
...
valid_data  <-readxl::read_excel("Interpre_RMA+QC - for R.xlsx",col_names=TRUE, na="NA", sheet="valid2")

#convert to spatial object:
valid_data2 <- valid_data

colnames(valid_data2) <- c( "Country"     ,             "WMO Code"   ,              "Site"     ,                "Latitude"   ,             
                           "Longitude",                "X",                        "Y",                        "Altitude",                 "All time O",              
                           "All time H",               "Focus period O",           "Focus period H",           "RMA_slope",                "RMA_intercept",           
                           "r2",                       "p",                        "Nr of pairs (O,H)",        "Nr of Months",             "Data from how many years",
                           "QC"  )
...
#Train_Data = full_Data %>% dplyr::select(RMA_slope, X, Altitude, KG) # response variables (RMA slope / RMA intercept) and other predictors.
#colnames(Train_Data) = c("RMA_slope", "x","y", "elevation", "KG")
...
#Step 0 - Use Buffer distances instead of geographical coordinates as covariants:
...
TuneRF = tune( RMA_slope ~ ., data = rm.RMA_slope, ntreeTry = 1000)
RF_buffer_distance = rfsrc( RMA_slope ~ ., data = rm.RMA_slope,
            nodesize = TuneRF$optimal['nodesize'],
            mtry = TuneRF$optimal['mtry'], importance = TRUE)
plot(RF_buffer_distance)

grid.dist0$KG <- as.factor(grid.dist0$KG)


Y0_hat_RF_buffer_distance = predict(RF_buffer_distance, newdata = grid.dist0@data, na.action = "na.impute")$predicted



Y0_hat_buffer_distance = X1 %>% cbind.data.frame( Pred_RF = Y0_hat_RF_buffer_distance)

coordinates(Y0_hat_buffer_distance) <- ~ x + y
proj4string(Y0_hat_buffer_distance) <- CRS("+init=epsg:3857")
gridded(Y0_hat_buffer_distance) <- TRUE
...
Y0_hat_buffer_distance_df$RMA_slope <- Y0_hat_buffer_distance$Pred_RF
...
#save the results:
library(xlsx)
write.xlsx(valid_data3,file="maps/final/v2/RMA_slope_results_x_y_KG_big_istvan.xlsx")
writeRaster(Raster1, filename="maps/final/v2/RMA_slope_x_y_KG_final.tif", overwrite = TRUE)
writeRaster(Y0_hat_buffer_distance_df, filename="maps/final/v2/RMA_slope_x_y_KG_Buffer_distance_final.tif",overwrite = TRUE)
...
#Random forest with distances /Hengl Method
#Train_Data = full_Data %>% dplyr::select(RMA_intercept, X, Altitude, KG) # response variables (RMA slope / RMA intercept) and other predictors.
#colnames(Train_Data) = c("RMA_intercept", "x","y", "elevation", "KG")
...
TuneRF = tune( RMA_intercept ~ ., data = rm.RMA_intercept, ntreeTry = 1000)
RF_buffer_distance = rfsrc( RMA_intercept ~ ., data = rm.RMA_intercept,
                            nodesize = TuneRF$optimal['nodesize'],
                            mtry = TuneRF$optimal['mtry'], importance = TRUE)
plot(RF_buffer_distance)
...
mapview(Y0_hat_buffer_distance, alpha=1)
...
Y0_hat_buffer_distance_df <- as.data.frame(GRID_coordinates)
Y0_hat_buffer_distance_df$RMA_intercept <- Y0_hat_buffer_distance$Pred_RF
coordinates(Y0_hat_buffer_distance_df) <- ~ x + y
proj4string(Y0_hat_buffer_distance_df) <- CRS("+init=epsg:3857")
gridded(Y0_hat_buffer_distance_df) <- TRUE
...
#save the results:
library(xlsx)
write.xlsx(valid_data3,file="maps/final/v2/RMA_intercept_results_x_y_KG_big_istvan.xlsx")
writeRaster(Raster1, filename="maps/final/v2/RMA_intercept_x_y_KG_final.tif", overwrite = TRUE)
writeRaster(Y0_hat_buffer_distance_df, filename="maps/final/v2/RMA_intercept_x_y_KG_Buffer_distance_final.tif",overwrite = TRUE)
